...
{
    internal sealed class AITextureAssetNodeUpdater : AssetNodePresenterUpdaterBase
    {
        ...
        {
            var asset = node.Asset?.Asset as AITextureAsset;
            if (asset != null && node.Name == nameof(AITextureAsset.Generate))
            {
                if (node.Asset is AITextureViewModel model)
                {
                    asset.OnGenerated = model.Generate;
                }
            }

            if (asset != null && node.Name == nameof(AITextureAsset.Width))
            {
        ...
        {
            var asset = root.Asset?.Asset as AITextureAsset;
            if (asset != null)
            {
                var size = CategoryData.ComputeCategoryNodeName("Size");
                root[size][nameof(AITextureAsset.Width)].AddDependency(root[size][nameof(TextureAsset.IsSizeInPercentage)], false);
                root[size][nameof(AITextureAsset.Height)].AddDependency(root[size][nameof(TextureAsset.IsSizeInPercentage)], false);
            }
        }
    }
}
