...
namespace Stride.Assets.Textures
{
    ...
    public sealed partial class AITextureAsset : AssetWithSource
    {
        private const string CurrentVersion = "2.0.0.0";

        /// <summary>
        /// The default file extension used by the <see cref="AITextureAsset"/>.
        /// </summary>
        public const string FileExtension = ".sdtexai";

        public AITextureAsset()
        {
            Generate = () =>
            {
                var buffer = GenerateImageFromChatGPT(Prompt).Result;
                File.WriteAllBytes(Source.FullPath, buffer);
                OnGenerated?.Invoke();
            };
        }
        ...
        [DefaultValue(true)]
        [Display("Prompt")]        
        public string Prompt { get; set; } = "A leather skin";
        ...
        [DefaultValue(true)]
        [Display("Prompt")]
        [DataMemberAction("Generate")]
        public Action Generate { get; set; }

        [DataMemberIgnore]
        public Action OnGenerated { get; set; }

        private async Task<byte[]> GenerateImageFromChatGPT(string prompt)
        {
            return File.ReadAllBytes(@"D:\PavingStones005.png");
        }
